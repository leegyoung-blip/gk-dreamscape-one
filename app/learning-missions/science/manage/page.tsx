...
export default function ScienceCurriculumManagerPage() {
  ...
  const [selectedStatus, setSelectedStatus] = useState<ScienceQuizStatus | "all">("all");
  ...
  async function loadManager() {
    ...
    if (levelResult.error || !levelResult.data) {
      setMessage(levelResult.error?.message || "Run the Science P1 migration first.");
      setLoading(false);
      return;
    }
    ...
    if (topicResult.error || quizResult.error) {
      setMessage(topicResult.error?.message || quizResult.error?.message || "Could not load Science curriculum.");
    }
  ...
  const visibleQuizzes = useMemo(() => {
    return quizzes.filter((quiz) => {
      if (selectedTopicId !== "all" && quiz.topic_id !== selectedTopicId) return false;
      if (selectedStatus !== "all" && quiz.status !== selectedStatus) return false;
      return true;
    });
  ...
  function startNewQuiz() {
    const topicId = selectedTopicId !== "all" ? selectedTopicId : topics[0]?.id || "";
    const nextSequence = Math.max(
      0,
      ...quizzes.filter((quiz) => quiz.topic_id === topicId).map((quiz) => quiz.sequence_no),
    ) + 1;
  ...
  return (
    <SciencePageShell>
      <header className="flex flex-wrap items-center justify-between gap-3">
        <Link
          href="/learning-missions/science/p1"
      ...
      <section className="mt-7 rounded-[2.2rem] border border-violet-200/18 bg-[linear-gradient(145deg,rgba(36,19,67,0.74),rgba(4,14,32,0.9))] p-6 shadow-[0_28px_84px_rgba(0,0,0,0.3)] backdrop-blur-xl sm:p-8">
        ...
        <h1 className="mt-3 text-4xl font-black tracking-[-0.05em] sm:text-6xl">
          P1 Science Curriculum
        </h1>
        <p className="mt-4 max-w-3xl text-base leading-7 text-white/58">
          Create quiz shells, enter questions, review content and publish approved missions. Regular teachers can teach and view progress but cannot edit this curriculum.
        </p>
      </section>
          ...
          <section className="mt-6 rounded-[1.8rem] border border-white/10 bg-white/[0.045] p-5 backdrop-blur-xl">
            <div className="grid gap-3 md:grid-cols-2">
              <label className="grid gap-2 text-xs font-black uppercase tracking-[0.12em] text-white/45">
                ...
                >
                  <option value="all">All P1 topics</option>
                  {topics.map((topic) => (
              ...
              <label className="grid gap-2 text-xs font-black uppercase tracking-[0.12em] text-white/45">
                ...
                <select
                  ...
                  onChange={(event) => setSelectedStatus(event.target.value as ScienceQuizStatus | "all")}
                  className="min-h-12 rounded-2xl border border-white/12 bg-[#07162c] px-4 text-sm font-bold normal-case tracking-normal text-white outline-none"
                >
            ...
            {visibleQuizzes.length === 0 ? (
              ...
              visibleQuizzes.map((quiz) => {
                ...
                return (
                  <article
                    key={quiz.id}
                    className="grid gap-4 rounded-3xl border border-white/10 bg-[linear-gradient(145deg,rgba(8,31,57,0.75),rgba(3,13,30,0.9))] p-5 md:grid-cols-[minmax(0,1fr)_auto] md:items-center"
                  >
                    <div className="min-w-0">
                      <div className="flex flex-wrap items-center gap-2">
                        <span className="rounded-full border border-cyan-200/18 bg-cyan-300/10 px-3 py-1 text-[10px] font-black uppercase tracking-[0.12em] text-cyan-100">
                          {mission.icon} {mission.shortLabel}
                        </span>
                        <span className={`rounded-full border px-3 py-1 text-[10px] font-black uppercase tracking-[0.12em] ${status.className}`}>
                          {status.label}
                        </span>
                      ...
                      <p className="mt-1 text-sm text-white/48">
                        {topic?.title || "P1 Topic"} · {quiz.question_target} questions · {quiz.estimated_minutes} min · Difficulty {quiz.difficulty}
                      </p>
                    </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center overflow-y-auto bg-black/75 p-4 backdrop-blur-md">
          <section className="w-full max-w-3xl rounded-[2rem] border border-violet-200/24 bg-[#071326] p-5 shadow-[0_36px_100px_rgba(0,0,0,0.6)] sm:p-7">
            ...
            <div className="mt-6 grid gap-4 md:grid-cols-2">
              ...
              <Field label="Mission type">
                <select
                  ...
                  onChange={(event) => setDraft((current) => ({ ...current, missionType: event.target.value as ScienceMissionType }))}
                  className={inputClass}
                >
                  {(Object.keys(SCIENCE_MISSION_META) as ScienceMissionType[]).map((type) => (
              ...
              <Field label="Quiz title" wide>
                <input
                  ...
                  className={inputClass}
                  placeholder="Example: What Can You Observe?"
                />
              </Field>
              ...
              <Field label="Description" wide>
                <textarea
                  ...
                  placeholder="Short student-facing description"
                />
              </Field>

              <NumberField label="Sequence number" value={draft.sequenceNo} min={1} max={999} onChange={(value) => setDraft((current) => ({ ...current, sequenceNo: value }))} />
              <NumberField label="Difficulty" value={draft.difficulty} min={1} max={5} onChange={(value) => setDraft((current) => ({ ...current, difficulty: value }))} />
              <NumberField label="Question target" value={draft.questionTarget} min={1} max={30} onChange={(value) => setDraft((current) => ({ ...current, questionTarget: value }))} />
              <NumberField label="Estimated minutes" value={draft.estimatedMinutes} min={1} max={120} onChange={(value) => setDraft((current) => ({ ...current, estimatedMinutes: value }))} />

              <Field label="Status" wide>
                <select
                  value={draft.status}
                  onChange={(event) => setDraft((current) => ({ ...current, status: event.target.value as ScienceQuizStatus }))}
                  className={inputClass}
                >
                  {(Object.keys(SCIENCE_STATUS_META) as ScienceQuizStatus[]).map((status) => (
                    <option key={status} value={status}>
            ...
            <div className="mt-6 flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
              ...
              >
                {saving ? "Saving…" : draft.id ? "Save changes" : "Create and add questions"}
              </button>
            </div>
          </section>
        </div>
