...
export function ArenaVersusCharacterSelect({
  ...
  // Prefer the designed rarity order, but never silently hide a future
  // server-enabled monster if the roster expands again.
  const orderedSlugs = new Set(orderedMonsters.map((monster) => monster.slug));
  const selectableMonsters = [
    ...orderedMonsters,
    ...monsters.filter((monster) => !orderedSlugs.has(monster.slug)),
  ];
  ...
  return (
    <div className="kavs-select-shell">
      ...
      <div className="kavs-choice-section">
        <div className="kavs-section-title">Arena monsters · {selectableMonsters.length} choices</div>
      ...
      <style jsx>{`
        ...
        .kavs-choice-section{margin-top:16px}.kavs-section-title{margin-bottom:9px;font-size:12px;font-weight:950;letter-spacing:.12em;text-transform:uppercase;color:rgba(255,255,255,.72)}.kavs-grid{display:grid;gap:10px}.kavs-nova-grid{grid-template-columns:repeat(4,minmax(0,1fr))}.kavs-monster-grid{grid-template-columns:repeat(5,minmax(0,1fr))}
