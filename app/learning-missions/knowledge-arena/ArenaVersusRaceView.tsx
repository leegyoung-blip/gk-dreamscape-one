...
import { getNovaVariant } from "./novaVariants";
...
function spriteFor(player:VersusRacePlayer, monsters:VersusRaceMonster[]) {
  ...
  return getNovaVariant(player.character_slug).sprite;
}
...
export function ArenaVersusRaceView({topic,question,questionIndex,timeLeft,answerLocked,selectedAnswer,feedback,getAnswerStyle,onChoose,players,monsters,myPlayerId,roundStatus}:{
  ...
  return <div className="kavr-stage" style={{backgroundImage:`url("${arenaBackgrounds[topic]||arenaBackgrounds.world_explorer}")`}}>
    ...
    <div className="kavr-question-row">
      <div className="kavr-question">
        ...
        {feedback&&answerLocked&&(
          <div className="kavr-feedback">
            <strong>{feedback}</strong>
            <span><b>Explanation:</b> {question.explanation}</span>
          </div>
        )}
      </div>
    ...
    <style jsx>{`
      ...
      .kavr-question-row{display:grid;grid-template-columns:1fr 1fr;gap:10px;padding:8px 13px}.kavr-question{border:1px solid rgba(126,232,255,.18);border-radius:16px;background:rgba(5,13,29,.84);padding:12px;backdrop-filter:blur(7px)}.kavr-question h2{margin:0;font-size:clamp(18px,2.15vw,31px);line-height:1.15}.kavr-question img{max-height:80px;max-width:100%;object-fit:contain;margin-top:8px}.kavr-feedback{display:grid;gap:5px;margin-top:8px;border-radius:10px;background:rgba(4,12,28,.88);padding:7px 9px;font-size:11px;font-weight:800}.kavr-feedback strong{font-size:11px}.kavr-feedback span{border-top:1px solid rgba(255,255,255,.12);padding-top:5px;color:rgba(255,255,255,.9);font-size:10px;line-height:1.35}.kavr-feedback b{color:#8beaff}.kavr-options{display:grid;grid-template-columns:1fr 1fr;gap:8px}.kavr-options button{min-height:66px;display:grid;grid-template-columns:auto 1fr;align-items:center;gap:8px;border-radius:14px;padding:9px 11px;text-align:left}.kavr-options strong{display:grid;width:28px;height:28px;place-items:center;border-radius:999px;background:rgba(255,255,255,.12)}.kavr-options span{font-weight:800}
