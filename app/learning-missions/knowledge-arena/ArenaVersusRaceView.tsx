...
export function ArenaVersusRaceView({topic,question,questionIndex,timeLeft,answerLocked,selectedAnswer,feedback,getAnswerStyle,onChoose,players,monsters,myPlayerId,roundStatus}:{
  ...
  return <div className="kavr-stage" style={{backgroundImage:`url("${arenaBackgrounds[topic]||arenaBackgrounds.world_explorer}")`}}>
    ...
    <div className="kavr-arena-divider" aria-hidden="true" />
    ...
    <style jsx>{`
      .kavr-stage{position:relative;display:grid;grid-template-rows:auto auto auto minmax(0,1fr);width:100%;height:100%;overflow:hidden;border-radius:18px;background-size:cover;background-position:center;color:white;isolation:isolate}.kavr-shade{position:absolute;inset:0;z-index:-1;background:linear-gradient(180deg,rgba(1,6,17,.56),rgba(1,6,17,.1) 48%,rgba(1,6,17,.55))}
      ...
      .kavr-arena-divider{height:2px;margin:0 13px 4px;border-radius:999px;background:linear-gradient(90deg,transparent,rgba(126,232,255,.68) 10%,rgba(126,232,255,.95) 50%,rgba(126,232,255,.68) 90%,transparent);box-shadow:0 0 10px rgba(126,232,255,.28)}
