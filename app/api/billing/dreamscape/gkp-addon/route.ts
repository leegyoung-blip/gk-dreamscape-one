...
function monthStart(value: string) {
  ...
  if (!Number.isFinite(date.getTime())) {
    throw new Error("Invalid Dreamscape add-on start date.");
  }
...
export async function POST(request: Request) {
  try {
    const staff = await requireBillingStaff(request);
    ...
    const planCode = body.planCode || "none";
    ...
    const { data: existingAddon, error: addonError } =
      await supabaseAdmin
      ...
      if (userId) {
        await supabaseAdmin
          ...
          .update({
            status: "revoked",
            revoked_at: endedAt,
            revoke_reason: "GKP Dreamscape add-on ended",
    ...
    if (!["core", "complete"].includes(planCode)) {
      return json({ error: "Invalid GKP Dreamscape plan." }, 400);
    }
    ...
    const { count: activeEnrolmentCount, error: enrolmentError } =
      await supabaseAdmin
    ...
    const { data: conflictingContract, error: conflictError } =
      await supabaseAdmin
    ...
    const monthlyFee = planCode === "complete" ? 14.9 : 9.9;
    const freeAlreadyUsed = Boolean(
      existingAddon?.free_month_used_at,
    );

    const applyFreeMonth =
      Boolean(body.firstMonthFree) && !freeAlreadyUsed;

    const complimentaryThroughPeriod = applyFreeMonth
      ? monthStart(startsOn)
      : null;

    const { data: addon, error: saveError } = await supabaseAdmin
      .from("gkp_dreamscape_student_addons")
      .upsert(
        {
          ...
          first_month_free:
            applyFreeMonth ||
            Boolean(existingAddon?.first_month_free),
          complimentary_through_period:
            complimentaryThroughPeriod ||
            existingAddon?.complimentary_through_period ||
            null,
          free_month_used_at:
            applyFreeMonth
              ? new Date().toISOString()
              : existingAddon?.free_month_used_at || null,
          created_by: existingAddon?.created_by || staff.id,
          updated_by: staff.id,
    ...
    console.error("GKP Dreamscape add-on update failed", error);
