...
type CreditStateRow = { balance_credits: number; lifetime_purchased_credits: number; lifetime_redeemed_credits: number };
type CreditRedeemRow = { accepted: boolean; credits_spent: number; credit_balance: number; battery_seconds: number; bonus_seconds: number; total_playable_seconds: number };

function firstRow<T>(data: T[] | T | null): T | null { return !data ? null : Array.isArray(data) ? (data[0] ?? null) : data; }
...
export async function getActivityLabCreditPacks(): Promise<ActivityLabCreditPack[]> {
  ...
  if (error) throw error;
  return (data ?? []).map((row) => ({
    packKey: String(row.pack_key), title: String(row.title), description: row.description ? String(row.description) : null,
    credits: Number(row.credits ?? 0), priceCents: Number(row.price_cents ?? 0), currency: String(row.currency || "sgd"),
  }));
}

export async function redeemActivityLabCredits(credits: number, idempotencyKey: string): Promise<ActivityLabCreditRedeemResult> {
  const { data, error } = await supabase.rpc("activity_lab_credit_redeem", { p_credits: credits, p_idempotency_key: idempotencyKey });
  if (error) throw error;
  ...
  return {
    accepted: Boolean(row.accepted), creditsSpent: Number(row.credits_spent ?? 0), creditBalance: Number(row.credit_balance ?? 0),
    batterySeconds: Number(row.battery_seconds ?? 0), bonusSeconds: Number(row.bonus_seconds ?? 0), totalPlayableSeconds: Number(row.total_playable_seconds ?? 0),
  };
}
