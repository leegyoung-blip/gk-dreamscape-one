...
export function getHitPayEnvironment():
  HitPayEnvironment {
  return normaliseEnvironment(
    process.env.HITPAY_ENVIRONMENT,
  );
}

export function getHitPayApiConfig(
...
export function getHitPayWebhookSalt(
...
export async function createHitPayPayNowRequest(
  input: {
    amount: number;
    currency: string;
  ...
  const response = await fetch(
    `${config.baseUrl}/v1/payment-requests`,
    {
      method: "POST",
      headers: {
        "Content-Type":
      ...
      body: JSON.stringify({
        ...
        purpose:
          `Guru Kids Pro invoice ${input.invoiceNumber}`,
        reference_number:
          input.invoiceNumber,
        metadata: {
          invoice_id:
  ...
  const request =
    payload as HitPayPaymentRequestResponse;

  const qrPayload =
    request.qr_code_data?.qr_code;
...
export async function getHitPayPaymentRequest(
  ...
  const config =
    getHitPayApiConfig(environment);

  const response = await fetch(
    `${config.baseUrl}/v1/payment-requests/${encodeURIComponent(
      requestId,
    )}`,
    {
      method: "GET",
...
export async function createHitPaySubscriptionPlan(
  input: {
    name: string;
    description: string;
    amount: number;
  ...
  return hitPayJsonRequest<HitPaySubscriptionPlanResponse>(
    "/v1/subscription-plan",
    {
      ...
      body: JSON.stringify({
        ...
        // HitPay's API permits up to 100 charges.
        // This keeps the subscription long-running
...
export async function createHitPayRecurringBilling(
  input: {
    planId: string;
  ...
  return hitPayJsonRequest<HitPayRecurringBillingResponse>(
    "/v1/recurring-billing",
    {
      ...
      body: JSON.stringify({
        ...
        /*
         * Public Dreamscape billing is anchored
         * to the customer's purchase/start date.
         */
        start_date:
          input.startDate,
        ...
        /*
         * Singapore recurring checkout:
         * allow the customer to choose either
         * Card or GIRO on HitPay.
         */
        payment_methods: [
          "card",
          "giro",
...
export async function updateHitPayRecurringBilling(
  input: {
    environment:
      HitPayEnvironment;
    recurringBillingId:
...
export async function hitPayQrDataUrl(
  qrPayload: string,
) {
...
export function validateHitPayWebhookSignature(
  input: {
    rawBody: string;
    signature: string;
    salt: string;
...
export function isHitPayEnvironment(
  value: string,
): value is HitPayEnvironment {
  return (
    value === "sandbox" ||
    value === "production"
  );
