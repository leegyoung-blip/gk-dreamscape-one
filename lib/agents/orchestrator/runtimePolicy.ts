...
function runtimeCandidate(
  ...
  if (
    ...
    return allowed
      ...
      : {
          ...
          reasons: [
            ...candidate.reasons,
            "system.wait contract is not active",
          ],
        };
  }
  ...
  if (
    ...
    reasons.push(
      "blocked after an earlier failure in this session",
    );
  }
  ...
  if (
    ...
    reasons.push(
      "runtime contract is not active",
    );
  }
  ...
  if (
    ...
    reasons.push(
      "runtime contract is not executable",
    );
  }
...
export async function runRuleBasedRuntimeDecision({
  ...
  const [
    ...
    await Promise.all([
      ...
      admin
        ...
        .select(
          "id,agent_user_id,simulation_day_index,status",
        )
        .eq(
          "id",
          sessionId,
  ...
  if (
    ...
    throw new Error(
      agentError?.message ||
      "Runtime policy agent was not found.",
    );
  }
  ...
  if (
    ...
    throw new Error(
      ...
      "Runtime policy assignment was not found.",
    );
  }
  ...
  if (
    ...
    throw new Error(
      ...
      "Runtime policy session was not found.",
    );
  }
  ...
  if (
    ...
    throw new Error(
      "Runtime policy requires an active agent.",
    );
  }
  ...
  if (
    ...
    throw new Error(
      "Runtime policy requires a started session.",
    );
  }

  const simulationDayIndex =
    Number(
      session.simulation_day_index,
    );
  ...
  if (
    ...
    throw new Error(
      "Runtime session has no valid simulation day.",
    );
  }
  ...
  const {
    ...
    await admin
      ...
      .select(
        "id,policy_key,version,engine_kind,status",
      )
      .eq(
        "id",
  ...
  if (
    ...
    throw new Error(
      "Orchestrator requires active RuleBasedPolicyV1.",
    );
  }
  ...
  if (
    observation.sections.length !==
    18
  ) {
    throw new Error(
      `Runtime policy requires an 18-source snapshot; received ${observation.sections.length}.`,
    );
  }
  ...
  const recalled =
    await recallAgentMemory({
      ...
      query:
        "goals preferences recent success failure economy spending saving tokens gems learning nova milo rover knowledge think categories",
  ...
  const [
    ...
    await Promise.all([
      admin
        ...
        .select(
          "action_key,version,status,execution_mode,adapter_key",
        )
        .eq(
          "version",
          1,
      ...
      admin
        .from(
          "agent_execution_adapter_versions",
        )
        .select(
          "adapter_key,status",
        )
  ...
  if (
    ...
    throw new Error(
      "RuleBasedPolicyV1 has no runtime-safe action, including system.wait.",
    );
  }

  const reasoningSummary =
    `${agent.agent_code} selected ${selected.actionKey}${
      selected.targetLabel
        ? ` targeting ${selected.targetLabel}`
        : ""
    } with score ${selected.score.toFixed(
      3,
    )}. Runtime filtering removed inactive, non-executable, unavailable-adapter and same-session blocked actions before dispatch.`;
    ...
    await admin.rpc(
      "agent_store_runtime_policy_decision",
      {
        ...
        p_policy_input_summary: {
          ...
          executionContractsRequired:
            true,

          syntheticEconomyEnabled:
            true,
        },

        p_recalled_memory_ids:
          baseDecision.recalledMemoryIds,
      },
    );
  ...
  if (
    ...
    throw new Error(
      storeError?.message ||
      "Runtime policy decision could not be persisted.",
    );
  }
